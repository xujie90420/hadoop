...
import bigdata.bean.PageViewsBean;
import bigdata.bean.VisitBean;
 ...
 *
 */
public class VisitViews {
	
	static class VisitViewsMapper extends Mapper<LongWritable, Text, Text, PageViewsBean>{
		...
		protected void map(LongWritable key, Text value,Context context)throws IOException, InterruptedException {
			...
			pvBean.set(fields[0], fields[1], fields[2], fields[3],fields[4], Integer.parseInt(fields[5]), fields[6], fields[7], fields[8], fields[9]);
			k.set(pvBean.getSession());
			context.write(k, pvBean);
		}
	}
	
	static class VisitViewsReducer extends Reducer<Text, PageViewsBean, NullWritable, VisitBean>{
		@Override
	...
	public static void main(String[] args) throws Exception {
		...
		job.setJarByClass(VisitViews.class);
		
		job.setMapperClass(VisitViewsMapper.class);
		job.setMapOutputKeyClass(Text.class);
		job.setMapOutputValueClass(PageViewsBean.class);
		
		job.setReducerClass(VisitViewsReducer.class);
		job.setOutputKeyClass(NullWritable.class);
		job.setOutputValueClass(VisitBean.class);
		
		FileInputFormat.setInputPaths(job, new Path(args[0]));
		FileOutputFormat.setOutputPath(job,new Path(args[1]));
		/*FileInputFormat.setInputPaths(job, new Path("f:/weblog/PageViewoutput"));
		FileOutputFormat.setOutputPath(job, new Path("f:/weblog/VisitViewoutput"));*/
